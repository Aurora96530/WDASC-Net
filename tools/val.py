...
@torch.no_grad()
def evaluate_new(model, dataloader, device):
    print('Evaluating...')
    model.eval()
    metrics = Metrics(dataloader.dataset.n_classes, dataloader.dataset.ignore_label, device)
    ...
    for batch in tqdm(dataloader):
        ...
        images = torch.from_numpy(batch['img']).float().cuda()
        labels = torch.from_numpy(batch['mask']).long().cuda()
...
def main(cfg, args):
    ...
    if not model_path.exists():
        ...
        # model_path = Path(cfg['SAVE_DIR']) / \
        #              f"SegFormer_DDR_20240102202219/SegFormer_MiT-B0_DDR_best.pth"

...
class RandomHorizontalFlip_multi:
    ...
    def __call__(self, sample):
        ...
        other_mask = sample['other mask']

        if random.random() < self.p:
            img = TF.hflip(img)
            other_img = TF.hflip(other_img)
            mask = TF.hflip(mask)
            other_mask = TF.hflip(other_mask)
        return {'img': img, 'other img': other_img, 'mask': mask, 'other mask': other_mask}
        # return {'img': img, 'other img': other_img, 'mask': mask}


class RandomVerticalFlip_multi:
    def __init__(self, p: float = 0.5) -> None:
        self.p = p
    ...
    def __call__(self, sample):
        ...
        if random.random() < self.p:
            img = TF.vflip(img)
            other_img = TF.vflip(other_img)
            mask = TF.vflip(mask)
            other_mask = TF.vflip(other_mask)
        # return {'img': img, 'other img': other_img, 'mask': mask}
        return {'img': img, 'other img': other_img, 'mask': mask, 'other mask': other_mask}

class RandomResizedCrop_multi:
    ...
    def __call__(self, sample):
        ...
        other_img = TF.resize(other_img, [nH, nW], TF.InterpolationMode.BILINEAR)
        mask = TF.resize(mask, [nH, nW], TF.InterpolationMode.NEAREST)
        other_mask = TF.resize(other_mask, [nH, nW], TF.InterpolationMode.NEAREST)
        ...
        x2 = x1 + tW
        img = img[:, y1:y2, x1:x2]
        other_img = other_img[:, y1:y2, x1:x2]
        mask = mask[:, y1:y2, x1:x2]
        other_mask = other_mask[:, y1:y2, x1:x2]

        # pad the image
        if img.shape[1:] != self.size:
            padding = [0, 0, tW - img.shape[2], tH - img.shape[1]]
            img = TF.pad(img, padding, fill=0)
            other_img = TF.pad(other_img, padding, fill=0)
            mask = TF.pad(mask, padding, fill=self.seg_fill)
            other_mask = TF.pad(other_mask, padding, fill=self.seg_fill)
        # return img, mask
        # return {'img': img, 'other img': other_img, 'mask': mask}
        return {'img': img, 'other img': other_img, 'mask': mask, 'other mask': other_mask}
...
class Normalize_multi:
    ...
    def __call__(self, sample):
        ...
        other_img = TF.normalize(other_img, self.mean, self.std)
        return {'img': img, 'other img': other_img, 'mask': mask, 'other mask': other_mask}
...
class Resize_multi:
    ...
    def __call__(self, sample):
        ...
        other_img = TF.resize(other_img, (alignH, alignW), TF.InterpolationMode.BILINEAR)
        mask = TF.resize(mask, (alignH, alignW), TF.InterpolationMode.NEAREST)
        other_mask = TF.resize(other_mask, (alignH, alignW), TF.InterpolationMode.NEAREST)
        return {'img': img, 'other img': other_img, 'mask': mask, 'other mask': other_mask}
...
def get_train_augmentation(size: Union[int, Tuple[int], List[int]],
                           ...
                           v_flip: bool = False):
    augs = []
    if h_flip: augs.append(RandomHorizontalFlip(p=0.5))
    if v_flip: augs.append(RandomVerticalFlip(p=0.5))
    augs.append(RandomResizedCrop(size, scale=(0.5, 2.0), seg_fill=seg_fill))
    augs.append(Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]))
    return Compose(augs)
    # return Compose([
...
def get_train_augmentation_t(size: Union[int, Tuple[int], List[int]],
                           seg_fill: int = 0,
                           h_flip: bool = True,
                           v_flip: bool = False):
    augs = []
    ...
    if v_flip: augs.append(RandomVerticalFlip(p=0.5))
    augs.append(RandomResizedCrop(size, scale=(0.5, 2.0), seg_fill=seg_fill))
    augs.append(Normalize([0.318, 0.199, 0.083], [0.299, 0.191, 0.085]))
    return Compose(augs)


def get_val_augmentation_t(size: Union[int, Tuple[int], List[int]]):
    return Compose([
        Resize(size),
        Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
...
def get_train_augmentation_multi(size: Union[int, Tuple[int], List[int]],
                           seg_fill: int = 0,
                           h_flip: bool = True,
                           v_flip: bool = False):
    augs = []
    if h_flip: augs.append(RandomHorizontalFlip_multi(p=0.5))
    if v_flip: augs.append(RandomVerticalFlip_multi(p=0.5))
    augs.append(RandomResizedCrop_multi(size, scale=(1.0, 1.0), seg_fill=seg_fill))
    augs.append(Normalize_multi([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]))
    return transforms.Compose(augs)

def get_val_augmentation_multi(size: Union[int, Tuple[int], List[int]]):
    return transforms.Compose([
        Resize_multi(size),
        Normalize_multi([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])

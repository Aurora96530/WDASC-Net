...
class UperNet_fa(BaseModel):
    ...
    def __init__(self, backbone: str = "MobileNetV3-large", class_num: int = 19):
        ...
        self.head_seg = UPerHead(in_channels=self.backbone.channels,
                                 ...
                                 scales=(1, 2, 3, 6))
        self.SRdecoder = UPerHead_fa(in_channels=self.backbone.channels,
                                 channel=32,
                                 num_classes=class_num,
                                 scales=(1, 2, 3, 6))
    ...
    def forward(self, x):
        ...
        if self.training:
            logits_seg = F.interpolate(logits_seg, scale_factor=2, mode='bilinear', align_corners=True)
            logits_edge = self.head_edge(f_x4, f_x8)
            logits_edge = F.interpolate(logits_edge, x.shape[-2:], mode='bilinear', align_corners=True)
            ...
            seg_weight = self.query(logits_sr_up, logits_seg)
            fusion_seg = seg_weight * logits_seg + logits_seg

            logits_so = self.head_so([f_x4, f_x8, f_x16, f_x32])
            logits_so = F.interpolate(logits_so, x.shape[-2:], mode='bilinear', align_corners=True)
            logits_so = F.interpolate(logits_so, scale_factor=2, mode='bilinear', align_corners=True)
            # return torch.cat([logits_seg, logits_so], dim=1), logits_edge
            return logits_seg, logits_so, logits_sr_up, fusion_seg, logits_edge
        return logits_seg.contiguous()


if __name__ == "__main__":
    net = UperNet_fa(backbone="MobileNetV3-large", class_num=8)
    net.cuda()
    net.train()

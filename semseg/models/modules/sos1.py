...
class ModifiedSOEM(nn.Module):
    def __init__(self, ignore_label=255, ratio=0.1, threshold=0.5):
        super().__init__()
        self.ignore_label = ignore_label
        self.ratio = ratio
        self.threshold = threshold
    ...
    def forward(self, loss: torch.Tensor, labels_seg: torch.Tensor, labels_so: torch.Tensor) -> torch.Tensor:
        ...
        # 如果选择的小目标损失数量不足，则从小目标区域中选择最大的损失
        if loss_hard_s.numel() < n_min:
            if loss_s.numel() <= n_min:
                loss_hard_s = loss_s
            else:
        ...
        # 如果选择的大目标损失数量不足，则从大目标区域中选择最大的损失
        if loss_hard_l.numel() < n_min:
            if loss_l.numel() <= n_min:
                loss_hard_l = loss_l
            else:
        ...
        # 计算平均损失
        loss_hard = (torch.sum(loss_hard_s) + torch.sum(loss_hard_l)) / (
                    loss_hard_s.numel() + loss_hard_l.numel())

        return loss_hard


class DynamicModifiedSOEM(nn.Module):
    def __init__(self, ignore_label=255, ratio=0.1, initial_threshold=0.5):
        super().__init__()
        self.ignore_label = ignore_label
        self.ratio = ratio
        self.initial_threshold = initial_threshold
        self.threshold = initial_threshold  # 初始阈值

    def forward(self, loss, labels_seg, labels_so):
        n_min = int(labels_seg[labels_seg != self.ignore_label].numel() * self.ratio)
        loss_flat = loss.contiguous().view(-1)
        labels_so_flat = labels_so.contiguous().view(-1)
        loss_s = loss_flat[labels_so_flat == 1]  # loss for small objects
        ...
        # 动态调整阈值
        self.threshold = self._dynamic_threshold(loss_s)
    ...
    def _dynamic_threshold(self, loss_s):
        # 根据损失值动态调整阈值的示例函数
        # 在实际情况中根据数据特征、模型输出等动态调整阈值
        return self.initial_threshold * (1 - torch.mean(loss_s))  # 示例：根据平均损失值动态调整阈值
...
class HierarchicalSegLoss(nn.Module):
    ...
    def __init__(self, loss_seg_fn, loss_hier_fn, ignore_label, is_top=False, is_hier=False, is_soem=False, ratio=0.1):
        ...
        if is_soem:
            ...
            # self.soem = DynamicModifiedSOEM(ignore_label=ignore_label, ratio=ratio, initial_threshold=2.5 if self.is_hier else 0.5)
            # self.soem = ModifiedSOEM(ignore_label=ignore_label, ratio=ratio, threshold=2.5 if self.is_hier else 0.5)

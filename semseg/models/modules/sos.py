...
class SOEM_new(nn.Module):

    def __init__(self, ignore_label=255, ratio=0.1, threshold=0.5, balance_ratio=0.7) -> None:
        """
        Small object example mining for SOSNet
        Args:
            ignore_label: int, ignore label id in dataset.
    ...
    def forward(self, loss: torch.Tensor, labels_seg: torch.Tensor, labels_so: torch.Tensor) -> torch.Tensor:
        ...
        # loss_hard = (torch.sum(loss_hard_s) + torch.sum(loss_hard_l)) / (loss_hard_s.numel() + loss_hard_l.numel())
        loss_hard = (torch.sum(loss_hard_s) * self.balance_ratio + torch.sum(loss_hard_l) * (
                    1 - self.balance_ratio)) / (loss_hard_s.numel() + loss_hard_l.numel())
...
class HierarchicalSegLoss_new(nn.Module):

    def __init__(self, loss_seg_fn, loss_hier_fn, ignore_label, is_top=False, is_hier=False, is_soem=False, ratio=0.1):
        super(HierarchicalSegLoss_new, self).__init__()
        self.loss_bottom_fn = loss_seg_fn
        self.loss_top_fn = loss_seg_fn
        self.loss_hier_fn = loss_hier_fn
        self.is_top = is_top
        ...
        self.is_soem = is_soem
        if is_soem:
            self.soem = SOEM_new(ignore_label=ignore_label, ratio=ratio, threshold=2.5 if self.is_hier else 0.5)
            # self.soem = SOEM(ignore_label=ignore_label, ratio=ratio, threshold=2.5 if self.is_hier else 0.5)

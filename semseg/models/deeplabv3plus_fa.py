...
#         self.conv_sr = nn.Conv2d(sr_ch, hidden_state, kernel_size=1)
#         self.conv_seg = nn.Conv2d(seg_ch, hidden_state, kernel_size=1)
...
#             nn.Conv2d(hidden_state * 2, hidden_state, kernel_size=1),
#             nn.ReLU(),
#             nn.Conv2d(hidden_state, 1, kernel_size=1),
#             nn.Sigmoid()
#         )
#         # self.fusion = nn.Conv2d(hidden_state, seg_ch, kernel_size=1)
#
#     def forward(self, sr_fe, seg_fe):
#         sr_fe = self.conv_sr(sr_fe)
#         seg_fe = self.conv_seg(seg_fe)
#         combined_fe = torch.cat([sr_fe, seg_fe], dim=1)
#         attention_weights = self.attention(combined_fe)
#         weighted_seg_fe = seg_fe * attention_weights
#         conv_sp1_fe = self.conv_sp1(weighted_seg_fe)
#         conv_sp2_fe = self.conv_sp2(weighted_seg_fe)
#
#         fused_fe = sr_fe + conv_sp1_fe + conv_sp2_fe
#         seg_fusion = self.fusion(fused_fe)
#         # concat = torch.cat([sr_fe, seg_fe], dim=1)
...
#
#
# class FSL1(nn.Module):
#     def __init__(self, sr_ch, seg_ch, hidden_state=32):
#         super(FSL1, self).__init__()
#         self.conv_sr = nn.Conv2d(sr_ch, hidden_state, kernel_size=1)
#         self.conv_seg = nn.Conv2d(seg_ch, hidden_state, kernel_size=1)
#         self.attention = nn.Sequential(
#             nn.Conv2d(hidden_state * 2, hidden_state, kernel_size=1),
...
#             nn.Conv2d(hidden_state, 1, kernel_size=1),
#         )
#         self.softmax = nn.Softmax(dim=2)  # Softmax along spatial dimension
#         self.fusion = nn.Conv2d(hidden_state, seg_ch, kernel_size=1)
#
#     def forward(self, sr_fe, seg_fe):
#         sr_fe = self.conv_sr(sr_fe)
...
#         combined_fe = torch.cat([sr_fe, seg_fe], dim=1)
#         attention_weights = self.attention(combined_fe)
#         attention_weights = self.softmax(attention_weights)  # Normalize attention weights
#         weighted_seg_fe = seg_fe * attention_weights
#         fused_fe = weighted_seg_fe + sr_fe
#         seg_fusion = self.fusion(fused_fe)
#         return seg_fusion


class DeeplabV3Plus_fa(BaseModel):
    def __init__(self, backbone='ResNet-50', class_num=19):
        super(DeeplabV3Plus_fa, self).__init__(backbone=backbone)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            return logits_bottom, logits_top, logits_sr_up, None
            # return logits_bottom, logits_top, logits_sr_up, self.pointwise(logits_bottom), logits_sr_up, None
...
if __name__ == "__main__":
    ...
    net = DeeplabV3Plus_fa(backbone="MobileNetV3-large", class_num=8)
    net.cuda()
    net.train()
